...
#define maxFoam 7
	...
	for (int casc=0; casc < cascadeCount; casc++) {
		jxx += texture(htk, vec3(DataIn.texCoord/L[casc], LAYER_JXX))[casc] * choppyFactor;
		jyy += texture(htk, vec3(DataIn.texCoord/L[casc], LAYER_JYY))[casc] * choppyFactor;
		jxy += texture(htk, vec3(DataIn.texCoord/L[casc], LAYER_VA_JXY))[casc] * choppyFactor;
	}
...
vec4 computeOceanColor(vec3 wn) {
	...
	//return skyC;
	return (1 - specCoef) * (heightColor+reflectColor) + vec4(spec) ;
}
